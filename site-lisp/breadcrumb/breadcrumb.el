...
(defadvice query-replace (before bc-query activate compile)
  "Intercept query-replace to save a breadcrumb bookmark before doing the replacement."
  (if bc-bookmark-hook-enabled
      (bc-set))
  )

(defadvice query-replace-regexp (before bc-query activate compile)
  "Intercept query-replace to save a breadcrumb bookmark before doing the replacement."
  (if bc-bookmark-hook-enabled
      (bc-set))
  )

(defadvice isearch-forward (before bc-search activate compile)
  "Intercept isearch to save a breadcrumb bookmark before doing the replacement."
  (if bc-bookmark-hook-enabled
      (bc-set))
  )

(defadvice isearch-backward (before bc-search activate compile)
  "Intercept isearch to save a breadcrumb bookmark before doing the replacement."
  (if bc-bookmark-hook-enabled
      (bc-set))
  )

(defadvice ido-find-file (before bc-find activate compile)
