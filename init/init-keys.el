...
(load-after 'sly
  ...
  (kd sly-mode-map
      '("C-c C-s"    	sly)
      '("C-c C-t"    	sly-edit-definition)
      '("C-c C-T"    	sly-pop-find-definition-stack)
      ...
      '("C-x SPC b"  	sly-eval-buffer)
      '("C-x SPC d"  	sly-eval-defun)
      '("C-x SPC e"  	sly-eval-interactive-eval)
      '("C-x SPC r"  	sly-eval-region)
      '("C-x SPC s"  	sly-eval-last-expression)
      '("C-x SPC SPC"	sly-eval-defun)
      '("C-x SPC p"  	sly-pprint-eval-last-expression)
      '("C-c C-e"    	sly-expand-1)
      '("C-c C-E"    	sly-macroexpand-all)
      '("C-c C-u"    	sly-undefine-function)
      ;; '("C-c E"   	sly-edit-value)
      ;; '("C-c C-l" 	sly-load-file)
      '("C-c C-g"    	sly-interrupt)
      ;; '("C-c M-d" 	sly-disassemble-symbol)
      ;; '("C-c C-t" 	sly-toggle-trace-fdefinition)
      ;; '("C-c C-t" 	sly-toggle-fancy-trace)
      ;; '("C-c I"   	sly-inspect)

      '("C-c d #"	common-lisp-hyperspec-lookup-reader-macro)
      '("C-c d ~"	common-lisp-hyperspec-format)
      '("C-c d a"	sly-apropos)
      '("C-c d d"	sly-describe-symbol)
      '("C-c d f"	sly-describe-function)
      '("C-c d h"	sly-documentation-lookup)
      '("C-c d p"	sly-apropos-package)
      '("C-c d z"	sly-apropos-all)

      '("C-c w a"	sly-who-specializes)
      '("C-c w b"	sly-who-binds)
      '("C-c w c"	sly-who-calls)
      '("C-c w m"	sly-who-macroexpands)
      '("C-c w r"	sly-who-references)
      '("C-c w s"	sly-who-sets)
      '("C-c w w"	sly-calls-who)

      '("C-c s c"	sly-list-callers)
      '("C-c s C"	sly-list-connections)
      '("C-c s d"	sly-db-pop-to-debugger-maybe)
      '("C-c s e"	sly-events-buffer)
      '("C-c s i"	sly-inferior-lisp-buffer)
      '("C-c s t"	sly-list-threads)

      ;; '("C-c M-i"	sly-fuzzy-complete-symbol)
      '("C-c ~"     	sly-mrepl-sync)
      '("C-c C-r"   	sly-mrepl)
      '("C-c C-x"   	sly-export-symbol-at-point)
      )
  ...
  (kd sly-editing-mode-map
      ...
      '("C-c C-b"   	sly-compile-and-load-file)
      '("C-c C-B"   	sly-compile-file)
